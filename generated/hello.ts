...
export namespace helloworld {
	export class HelloRequest extends pb_1.Message {
		...
		static fromObject(data: { name?: string }): HelloRequest {
	...
	export class HelloReply extends pb_1.Message {
		...
		static fromObject(data: { message?: string }): HelloReply {
	...
	export abstract class UnimplementedGreeterService {
		static definition = {
			SayHello: {
				...
				requestSerialize: (message: HelloRequest) =>
					Buffer.from(message.serialize()),
				requestDeserialize: (bytes: Buffer) =>
					HelloRequest.deserialize(new Uint8Array(bytes)),
				responseSerialize: (message: HelloReply) =>
					Buffer.from(message.serialize()),
				responseDeserialize: (bytes: Buffer) =>
					HelloReply.deserialize(new Uint8Array(bytes)),
			},
			SayHelloStreamReply: {
		...
		abstract SayHello(
			call: grpc_1.ServerUnaryCall<HelloRequest, HelloReply>,
			callback: grpc_1.sendUnaryData<HelloReply>,
		): void;
		abstract SayHelloStreamReply(
			call: grpc_1.ServerWritableStream<HelloRequest, HelloReply>,
		): void;
	}
	export class GreeterClient extends grpc_1.makeGenericClientConstructor(
		UnimplementedGreeterService.definition,
		"Greeter",
		{},
	) {
		constructor(
		...
		SayHello: GrpcUnaryServiceInterface<HelloRequest, HelloReply> = (
			message: HelloRequest,
			metadata:
				| grpc_1.Metadata
				| grpc_1.CallOptions
				| grpc_1.requestCallback<HelloReply>,
			options?: grpc_1.CallOptions | grpc_1.requestCallback<HelloReply>,
			callback?: grpc_1.requestCallback<HelloReply>,
		): grpc_1.ClientUnaryCall => {
			return super.SayHello(message, metadata, options, callback);
		};
		SayHelloStreamReply: GrpcStreamServiceInterface<
			HelloRequest,
			HelloReply
		> = (
			message: HelloRequest,
			...
			options?: grpc_1.CallOptions,
		): grpc_1.ClientReadableStream<HelloReply> => {
