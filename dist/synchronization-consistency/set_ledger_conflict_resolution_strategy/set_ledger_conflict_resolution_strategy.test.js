...
test("set ledger conflict resolution strategy", (done) => {
    client.SetLedgerConflictResolutionStrategy(new sync.Strategy({ strategy: "latest" }), (err, response) => {
        done();
        expect(err).toBeNull();
        expect(response?.succeeded).toEqual(true);
    });
    client.SetLedgerConflictResolutionStrategy(new sync.Strategy({ strategy: "whatever" }), (err, response) => {
        done();
        expect(err).toBeNull();
