...
export const ForceLedgerReconciliation: handleUnaryCall<
	sync.Null,
	sync.Status
> = (call, callback) => {
	try {
		...
		if (missingInLocal.length > 0) {
			missingInLocal.forEach((id) => {
				const transaction = global_ledger.find(
					(tx) => tx.transaction_id === id,
				);
				if (transaction) local_ledger.push(transaction);
			});
		}
		...
		if (extraInLocal.length > 0) {
			extraInLocal.forEach((id) => {
				const index = local_ledger.findIndex(
					(tx) => tx.transaction_id === id,
				);
				if (index !== -1) local_ledger.splice(index, 1);
			});
		}
